...
<ul class="nav nav-tabs">
	...
	<?php  if($type['name'] == 'home') { ?><li<?php  if($do == 'display') { ?> class="active"<?php  } ?>><a href="<?php  echo url('site/nav/home', array('m' => $m, 'multiid' => $_GPC['multiid'], 'f' => $_GPC['f']));?>">微站首页导航图标</a></li><?php  } ?>
	<?php  if($type['name'] == 'profile') { ?><li<?php  if($do == 'display') { ?> class="active"<?php  } ?>><a href="<?php  echo url('site/nav/profile', array('m' => $m, 'multiid' => $_GPC['multiid'], 'f' => $_GPC['f']));?>">个人中心功能条目</a></li><?php  } ?>
	<?php  if(empty($module)) { ?>
	<li<?php  if($do == 'post' && empty($id)) { ?> class="active"<?php  } ?>><a href="<?php  echo url('site/nav/' . $type['name'], array('foo' => 'post', 'multiid' => $_GPC['multiid'], 'f' => $_GPC['f']));?>"><i class="fa fa-plus"></i> 添加条目</a></li>
	<?php  } ?>
	<?php  if($do == 'post' && !empty($id)) { ?>
	<li class="active"><a href="<?php  echo url('site/nav/' . $type['name'], array('id' => $id, 'foo' => 'post', 'multiid' => $_GPC['multiid'], 'f' => $_GPC['f']));?>"><i class="fa fa-edit"></i> 编辑条目</a></li>
	<?php  } ?>
</ul>
<?php  if($do == 'post') { ?>
				...
				<div class="col-sm-9 col-xs-12">
					<select class="form-control" name="multid">
						<?php  if(is_array($multis)) { foreach($multis as $multi) { ?>
							<option value="<?php  echo $multi['id'];?>" <?php  if($item['multiid'] == $multi['id'] || $_GPC['multiid'] == $multi['id']) { ?>selected<?php  } ?>><?php  echo $multi['title'];?><?php  if($multi['id'] == $default_site) { ?>[默认微站]<?php  } ?></option>
						<?php  } } ?>
					</select>
		...
		<div class="table-responsive panel-body" style="overflow: visible">
			<table class="table table-hover">
					...
					<tr>
						...
						<td class="text-center" style="width:100px;">
							<div class="text-center" >
								<a href="<?php  echo url('site/nav/'.$_GPC['do'], array('id' => $item['id'],'foo' =>'post', 'multiid' => $_GPC['multiid'], 'f' => $_GPC['f']));?>" class="btn btn-default btn-sm">编辑</a>
				...
				<tr>
					...
					<td class="text-center" style="width:200px; white-space:nowrap;overflow: visible" >
						<?php  if($item['remove']) { ?>
						<?php  echo tpl_form_field_link('url['.$item['id'].']', $item['url'], array('css' => array('input' => ' input-sm', 'btn' => ' btn-sm')));?>
						<?php  } else { ?>
						<?php  echo $item['url'];?>
