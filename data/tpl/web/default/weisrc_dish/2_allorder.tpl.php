                ...
                <table class="table table-hover">
                    ...
                    <tr>
                        ...
                        <td style="text-align:left;">
                            <a class="btn btn-info btn-sm" href="<?php  echo $this->createWebUrl('allorder', array('op' => 'detail', 'id' => $item['id'], 'storeid' => $storeid))?>" title="详情">详情</a>
                            <?php  if($item['status'] != -1) { ?>
                            <a class="btn btn-warning btn-sm" href="<?php  echo $this->createWebUrl('allorder', array('op' => 'print', 'id' => $item['id'], 'storeid' => $storeid))?>" title="打印订单" onclick="return confirm('确认打印吗？');return false;">打印</a><?php  } ?>
                            <?php  if($item['ispay'] == 0 &&  $item['status'] != -1) { ?>
                            <a class="btn btn-danger btn-sm" href="<?php  echo $this->createWebUrl('allorder', array('op' => 'detail', 'id' => $item['id'], 'storeid' => $storeid, 'confrimpay' => 'confrimpay'))?>"  onclick="return confirm('确认设置该订单为完成支付吗？'); return false;" title="付款">付款</a><?php  } ?>
                            <?php  if($item['status'] == 0) { ?>
                            <a class="btn btn-info btn-sm" href="<?php  echo $this->createWebUrl('allorder', array('op' => 'detail', 'id' => $item['id'], 'storeid' => $storeid, 'confirm' => 'confirm'))?>"  onclick="return confirm('确认设置该订单为已确认吗？'); return false;" title="确认">确认</a><?php  } ?>
                            <?php  if($item['status'] == 1) { ?>
                            <?php  if($item['ispay'] == 0) { ?>
                            <a class="btn btn-success btn-sm" href="#"  onclick="alert('请先支付订单，再完成订单');return false;" title="完成">完成</a><?php  } else { ?><a class="btn btn-success btn-sm" href="<?php  echo $this->createWebUrl('allorder', array('op' => 'detail', 'id' => $item['id'], 'storeid' => $storeid, 'finish' => 'finish'))?>"  onclick="return confirm('确认设置该订单为完成支付吗？'); return false;" title="完成">完成</a>
                            <?php  } ?>
                            <?php  } ?>
                            <?php  if($item['status'] != -1) { ?>
                            <a class="btn btn-default btn-sm" href="<?php  echo $this->createWebUrl('allorder', array('op' => 'detail', 'id' => $item['id'], 'storeid' => $storeid, 'close' => 'close'))?>"  onclick="return confirm('确认取消订单吗？'); return false;" title="取消">取消</a>
                            <?php  } ?>
                        ...
                        <form class="form-horizontal form" action="" method="post" enctype="multipart/form-data">
                                ...
                                <div class="form-group clearfix">
                                    ...
                                    <div class="col-xs-9 col-sm-9 form-control-static fix">
                                        <p class="js-receive-address" order-id="164">
                                            <span><?php  echo $item['username'];?> <?php  echo $item['tel'];?> <?php  echo $item['address'];?>(<a href="<?php  echo $this->createWebUrl('allfans', array('id' => $fans['id'], 'op' => 'post', 'storeid' => $storeid))?>">查看用户</a>)</span>
                                        </p>
                                    </div>
                                </div>
                            ...
                            <div class="col-xs-12 col-sm-6">
                                ...
                                <div class="form-group clearfix js-fee">
                                    ...
                                    <div class="col-sm-3 col-xs-12">
                                        <button type="submit" class="btn btn-primary span2" name="confirmprice" value="yes" onclick="return confirm('确认操作？');">修改</button>
                                    </div>
                                </div>
                                <?php  if($item['credit']) { ?>
                                <div class="form-group clearfix js-fee">
